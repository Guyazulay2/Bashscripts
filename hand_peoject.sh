...
if [ $Choice == "1" ]; then
    ...
    echo "** Cloning MediaPipe Repo **" ; https://github.com/dspip/Hand.git /$HOME
    cd /$HOME/Hand ; sudo bash ./setup_opencv.sh

fi

elif [ $Choice == "3" ];
    echo '** Copmile Hand-Counting App **'
    cd /$HOME/Hand ; bazel build -c opt --copt -DMESA_EGL_NO_X11_HEADERS --copt -DEGL_NO_X11 mediapipe/examples/desktop/hand_tracking:hand_tracking_out_gpu --verbose_failures
